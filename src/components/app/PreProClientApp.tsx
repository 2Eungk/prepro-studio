...
type PreProClientAppProps = {
  initialWorkspace?: PreProWorkspaceId;
};

export default function PreProClientApp({ initialWorkspace = 'shoot' }: PreProClientAppProps) {
  const initialTab = defaultTabByWorkspace[initialWorkspace] || 'schedule';
  const { 
  ...
  const [activeTab, setActiveTabState] = useState<MainWorkspaceTab>(initialTab);
  const [activeWorkspace, setActiveWorkspace] = useState<PreProWorkspaceId>(initialWorkspace);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const nextTab = isMainWorkspaceTab(urlTab) ? urlTab : null;
